...
The remainder of this report is organized as follows. Section~\ref{sec:preliminaries} covers preliminaries on policy gradient methods and PPO. Section~\ref{sec:methodology} describes the APG methodology in detail. Section~\ref{sec:environments} presents the five benchmark environments. Section~\ref{sec:experiments} details the experimental setup and provides result templates. Section~\ref{sec:conclusion} discusses findings and concludes.
...
We evaluate on five continuous-control environments of increasing complexity. The suite begins with two low-dimensional differentiable sanity checks, Continuous Mountain Car and PointMassSimple, followed by PointMassNavigate, PushT, and FrankaReach. Each benchmark is paired with a black-box PPO-compatible mode and a differentiable APG-compatible mode, either through a native PyTorch implementation or a custom gradient bridge.

\subsection{DiffMountainCarContinuous-v0}
...
\begin{table}[htbp]
  \centering
  \caption{DiffMountainCarContinuous-v0 observation space.}
  \label{tab:mc_obs}
  \begin{tabular}{@{}lll@{}}
    \toprule
...
\subsection{PointMassSimple-v0}
\label{sec:env_pointmass_simple}

\paragraph{Task:} Control a one-dimensional damped point mass so that its position tracks a randomly sampled target.

\paragraph{State space} (3-dimensional): See Table~\ref{tab:pms_obs}.

\begin{table}[htbp]
  \centering
  \caption{PointMassSimple-v0 observation space.}
  \label{tab:pms_obs}
  \begin{tabular}{@{}lll@{}}
    \toprule
    \textbf{Component} & \textbf{Dimensions} & \textbf{Description} \\
    \midrule
    Position & 1 & Scalar position $x$ \\
    Velocity & 1 & Scalar velocity $v$ \\
    Target   & 1 & Scalar target position $x_g$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\paragraph{Reset distribution:} Initial position and target are sampled uniformly from $[-1,1]$, while initial velocity is sampled from $[-0.25,0.25]$.

\paragraph{Dynamics} (semi-implicit Euler, $\Delta t = 0.1$):
\begin{align}
  f_t &\leftarrow \text{clip}(a_t, -1, 1) \cdot f_{\max} \\
  \ddot{x}_t &\leftarrow f_t - c_d \dot{x}_t \\
  \dot{x}_{t+1} &\leftarrow \dot{x}_t + \ddot{x}_t \Delta t \\
  x_{t+1} &\leftarrow x_t + \dot{x}_{t+1}\Delta t
\end{align}
where $f_{\max}=1.0$ and $c_d=0.5$.
...
\paragraph{Success condition:} The current implementation does not use a separate success termination; episodes end by timeout.

\paragraph{Episode length:} 50 steps in the standalone environment implementation; in the unified training script the horizon can be overridden through \texttt{-{}-max\_episode\_steps}.

\paragraph{APG gradient path:} $a_t \rightarrow f_t \rightarrow \dot{x}_{t+1} \rightarrow x_{t+1} \rightarrow r_t$.
...
Tables~\ref{tab:env_comparison} and~\ref{tab:env_comparison_rest} compare the five environments across key properties.

\begin{table}[htbp]
  \centering
  \caption{Comparison of environment properties, Part I: point-mass environments.}
  \label{tab:env_comparison}
  \small
  \begin{tabularx}{\linewidth}{@{}l>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X@{}}
    \toprule
    \textbf{Property} & \textbf{PointMassSimple} & \textbf{PointMassNavigate} \\
    \midrule
    Observation dim        & 3                         & 14 \\
    Action dim             & 1                         & 2 \\
    Dynamics               & 1D damped point mass      & 2D point mass (Euler) \\
    DOF                    & 1 (pos)                   & 2 (pos) \\
    Max episode steps      & 50                        & 100 \\
    Gradient bridge        & Native PyTorch            & Native PyTorch \\
    Obstacles/constraints  & None                      & 2 circles \\
    Orientation            & No                        & No \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[htbp]
  \centering
  \caption{Comparison of environment properties, Part II: remaining environments.}
  \label{tab:env_comparison_rest}
  \small
  \begin{tabularx}{\linewidth}{@{}l>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X@{}}
    \toprule
    \textbf{Property} & \textbf{MountainCar} & \textbf{PushT} & \textbf{FrankaReach} \\
    \midrule
    Observation dim        & 2                         & 9                         & 28 \\
    Action dim             & 1                         & 3                         & 7 \\
    Dynamics               & 1D hill car               & 2D rigid body (Euler)     & Articulated body (Featherstone) \\
    DOF                    & 1 (pos)                   & 3 (pos + rot)             & 7 (joints) \\
    Max episode steps      & 30                        & 100                       & 30 \\
    Gradient bridge        & Native PyTorch            & Native PyTorch            & Warp $\leftrightarrow$ PyTorch \\
    Obstacles/constraints  & Terrain slope + bounds    & Workspace bounds          & Joint limits \\
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    Custom env max episode steps    & 30 \\
...
\paragraph{MountainCar benchmark pairing.} PPO uses Gymnasium's black-box \texttt{MountainCarContinuous-v0}, while APG uses \texttt{DiffMountainCarContinuous-v0}, a torch-native differentiable reimplementation with matched state, action, and transition structure. Unless otherwise noted, APG MountainCar runs use the smooth reward mode described in Section~\ref{sec:env_mountaincar}; the original Gym reward is retained as an alternate setting for faithfulness checks.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}lcrrr@{}}
    ...
    PointMassSimple-v0   & \tbd & \tbd & \tbd & \tbd \\
...
\begin{table}[htbp]
  \centering
  \caption{Ablation study: effect of APG segment length and bootstrap mode on episodic return, Part I: point-mass environments.}
  \label{tab:ablation_seg}
  \small
  \begin{tabular}{@{}clcc@{}}
    \toprule
    \textbf{Segment Length} & \textbf{Bootstrap} & \textbf{PointMassSimple} & \textbf{PointMassNavigate} \\
    \midrule
    Full episode & ---       & \tbd & \tbd \\
    10           & MC        & \tbd & \tbd \\
    10           & Critic    & \tbd & \tbd \\
...
\begin{table}[htbp]
  \centering
  \caption{Ablation study: effect of APG segment length and bootstrap mode on episodic return, Part II: remaining environments.}
  \label{tab:ablation_seg_rest}
  \small
  \begin{tabular}{@{}clccc@{}}
    \toprule
    \textbf{Segment Length} & \textbf{Bootstrap} & \textbf{MountainCar} & \textbf{PushT} & \textbf{FrankaReach} \\
    \midrule
...
We presented a unified framework for comparing Analytic Policy Gradients (APG) and Proximal Policy Optimization (PPO) across five continuous-control environments, including two low-dimensional differentiable sanity-check benchmarks: Continuous Mountain Car and PointMassSimple. The design of the framework surfaces four key research questions whose empirical answers are deferred pending completed experiments:
...
\begin{enumerate}
  ...
  \item \textbf{Scalability with dynamics complexity:} APG's advantage is expected to be most pronounced in smooth, low-dimensional environments (MountainCarContinuous, PointMassSimple, and PointMassNavigate) and may diminish in high-DOF articulated-body settings (FrankaReach) where long gradient chains through forward kinematics can introduce instability.
...
\begin{enumerate}
  ...
  \item \textbf{Extended benchmark suite:} Evaluation on additional environments beyond MountainCar, PointMassSimple, PointMassNavigate, PushT, and FrankaReach (e.g., locomotion or contact-rich manipulation) would further characterize APG's strengths and limitations.
\end{enumerate}
