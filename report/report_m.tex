...
We evaluate on four continuous-control environments of increasing complexity. The suite begins with a low-dimensional differentiable reimplementation of Continuous Mountain Car, followed by PointMassNavigate, PushT, and FrankaReach. Each benchmark is paired with a black-box PPO-compatible mode and a differentiable APG-compatible mode, either through a native PyTorch implementation or a custom gradient bridge.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{@{}l>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X@{}}
    ...
    Action dim             & 1                         & 2                          & 3                          & 7 \\
    Dynamics               & 1D hill car              & Point mass (Euler)         & 2D rigid body (Euler)      & Articulated body (Featherstone) \\
    DOF                    & 1 (track position)        & 2 (pos)                    & 3 (pos + rot)              & 7 (joints) \\
    Max episode steps      & 200                       & 100                        & 100                        & 30 \\
    Gradient bridge        & Native PyTorch            & Native PyTorch             & Native PyTorch             & Warp $\leftrightarrow$ PyTorch \\
    Obstacles/constraints  & Terrain slope + bounds    & 2 circles                  & Workspace bounds           & Joint limits \\
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}clcccc@{}}
    ...
    \midrule
    Full episode & ---       & \tbd & \tbd & \tbd & \tbd \\
    10           & MC        & \tbd & \tbd & \tbd & \tbd \\
    10           & Critic    & \tbd & \tbd & \tbd & \tbd \\
    5            & MC        & \tbd & \tbd & \tbd & \tbd \\
    5            & Critic    & \tbd & \tbd & \tbd & \tbd \\
    \bottomrule
  \end{tabular}
\end{table}
...
We presented a unified framework for comparing Analytic Policy Gradients (APG) and Proximal Policy Optimization (PPO) across four continuous-control environments, including a low-dimensional differentiable Continuous Mountain Car benchmark. The design of the framework surfaces four key research questions whose empirical answers are deferred pending completed experiments:
...
\begin{enumerate}
  ...
  \item \textbf{Scalability with dynamics complexity:} APG's advantage is expected to be most pronounced in smooth, low-dimensional environments (MountainCarContinuous and PointMassNavigate) and may diminish in high-DOF articulated-body settings (FrankaReach) where long gradient chains through forward kinematics can introduce instability.
...
\begin{enumerate}
  ...
  \item \textbf{Extended benchmark suite:} Evaluation on additional environments beyond MountainCar, PointMass, PushT, and FrankaReach (e.g., locomotion or contact-rich manipulation) would further characterize APG's strengths and limitations.
\end{enumerate}
