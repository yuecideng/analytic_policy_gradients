...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[b]{0.32\textwidth}
    ...
    \caption{FrankaReach}
  \end{subfigure}
  \caption{Episodic return (mean $\pm$ std) vs.\ total gradient steps for PPO (blue) and APG (orange) across three environments. The x-axis is equalized: both algorithms execute the same total number of gradient updates.}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}lcrrr@{}}
    \toprule
    \textbf{Environment} & \textbf{Success Ratio} & \textbf{PPO Steps} & \textbf{APG Steps} & \textbf{APG Speedup} \\
    \midrule
...
Figures~\ref{fig:curve_pointmass}--\ref{fig:curve_franka} show the per-environment episodic return curves (mean $\pm$ std) at larger scale.
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[b]{0.3\textwidth}
    ...
    \caption{PointMassNavigate v0: blue dot = agent, green star = goal, pink circles = obstacles.}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.3\textwidth}
    \includegraphics[width=\textwidth]{../figures/push_t_env}
    \caption{PushT v0: dark gray = T-block (agent-controlled), light green = target pose.}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.35\textwidth}
    \includegraphics[width=\textwidth]{../figures/franka_env}
    \caption{FrankaReach v0: 32 parallel Franka Emika Panda arms simulated via Newton physics.}
  \end{subfigure}
  \caption{Rendered frames from the three custom differentiable environments used in experiments.}
  \label{fig:env_vis}
\end{figure}
