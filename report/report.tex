...
Figure~\ref{fig:ablation_bootstrap} shows the learning curves. Both modes reach a success rate of $1.0$ by the end of training. The critic bootstrap converges to a slightly higher final episodic return ($-14.44 \pm 3.38$) than MC bootstrap ($-15.31 \pm 2.84$), though the difference is within one standard deviation given three seeds. The critic bootstrap exhibits marginally faster early-phase improvement, consistent with the expected benefit of a learned value function reducing variance in the return estimate at segment boundaries.

\begin{figure}[htbp]
  \centering
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{../figures/ablation_bootstrap_return}
    \caption{Episodic return (mean $\pm$ std, 3 seeds).}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{../figures/ablation_bootstrap_success}
    \caption{Success rate (mean $\pm$ std, 3 seeds).}
  \end{subfigure}
  \caption{Bootstrap mode ablation on PointMassNavigate with segment length $L=25$. Blue: MC bootstrap; orange: critic bootstrap. Both modes use the same APG training loop; the only difference is how the segment boundary value $b_k$ is estimated.}
...
Figure~\ref{fig:ablation_segment} and Table~\ref{tab:ablation_segment} summarize the results.
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[b]{0.48\textwidth}
    ...
    \caption{Episodic return vs.\ global steps.}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{../figures/ablation_segment_success}
    \caption{Success rate vs.\ global steps.}
  \end{subfigure}
  \caption{Segment length ablation on PointMassNavigate. Each panel shows one bootstrap mode (MC left, Critic right); line styles encode $L \in \{10, 25, 50\}$ (solid, dashed, dotted). MC bootstrap degrades gracefully as $L$ shrinks; critic bootstrap collapses at $L=10$ and recovers only at $L=50$.}
...
We presented a unified framework for comparing Analytic Policy Gradients (APG) and Proximal Policy Optimization (PPO) across four differentiable continuous control environments. Experiments on PointMassNavigate, PushT, and FrankaReach yield four concrete findings:

\begin{enumerate}
  \item \textbf{Gradient quality vs.\ variance:} APG computes exact policy gradients by backpropagating through dynamics, bypassing the variance inherent in Monte Carlo advantage estimation. Across all three tested environments APG converges to a success rate of 1.0, confirming that exact gradients provide a reliable training signal.

  \item \textbf{Sample efficiency:} APG requires substantially fewer gradient steps than PPO to reach comparable performance thresholds (Table~\ref{tab:grad_eff}): 9.2$\times$ fewer on PointMassNavigate, 3.9$\times$ on PushT, and 15.9$\times$ on FrankaReach, demonstrating that the per-step quality of analytic gradients more than compensates for PPO's multi-epoch update schedule.
