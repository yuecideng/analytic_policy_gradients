...
def plot_ax(ax, grouped_env, tb_key, grad_key, ylabel, title, use_grad_steps=False):
    ...
    for mode in ("mc", "critic"):
        ...
        ax.plot(steps, mean, label=label, color=style["color"],
                linestyle=style["ls"], linewidth=2)
        ax.fill_between(steps, mean - std, mean + std,
                        color=style["color"], alpha=0.2)
        summary_rows.append((mode, n, mean[-1], std[-1]))
        print(f"    [{mode}] {n} seeds, {len(steps)} pts, "
...
for tb_key, grad_key, ylabel, fig_stem, title_suffix in METRICS:
    for x_axis in ("steps", "gradsteps"):
        ...
        n_envs = len(env_ids)
        fig, axes = plt.subplots(1, max(n_envs, 1),
                                 figsize=(5 * max(n_envs, 1), 4),
                                 squeeze=False)
        axes = axes[0]
        ...
        for ax, env_id in zip(axes, env_ids):
            ...
            rows = plot_ax(ax, grouped[env_id], tb_key, grad_key,
                           ylabel, env_title, use_grad_steps=use_grad)
