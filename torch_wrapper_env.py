...
class TorchWrapperEnv:
    ...
    def step(self, action):
        ...
        # Handle discrete actions: argmax if one-hot / soft vector
        if isinstance(self.single_action_space, gym.spaces.Discrete):
            if action_np.ndim > 1 and action_np.shape[-1] == self.single_action_space.n:
                action_np = action_np.argmax(axis=-1)
