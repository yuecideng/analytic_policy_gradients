...
# Franka FR3 arm has 7 actuated joints (URDF has no gripper)
...
DEFAULT_JOINT_Q = [0.0, -0.7854, 0.0, -2.3562, 0.0, 1.5708, 0.7854]
...
class FrankaReachEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        wp.copy(self.state_0.joint_q, wp.array(joint_q_np, dtype=wp.float32, device=self.model.device))
        wp.copy(self.state_0.joint_qd, wp.array(joint_qd_np, dtype=wp.float32, device=self.model.device))
        wp.copy(self.control.joint_target_pos, wp.array(joint_q_np, dtype=wp.float32, device=self.model.device))
    ...
    def _get_reward(self):
        ...
        return _compute_reward(
            eef_pos.reshape(1, 3), eef_quat.reshape(1, 4),
            self.target_pos, self.target_quat, self.w_rot,
        )[0]
...
def _set_joint_targets_kernel(
    ...
    target_pos: wp.array(dtype=wp.float32),      # control.joint_target_pos
...
def _compute_reward_kernel(
    ...
    env_idx = wp.tid()
    ee_global = body_world_start[env_idx] + ee_body_offset
...
class _NewtonStepFunc(torch.autograd.Function):
    ...
    def forward(ctx, action_torch, sim_state):
        ...
        body_ws_wp = sim_state["body_world_start_wp"]
        ...
        target_pos_wp = wp.array(target_pos_np.tolist(), dtype=wp.vec3f, device=model.device)
        reward_wp = wp.zeros(num_envs, dtype=wp.float32, device=model.device, requires_grad=True)
        ...
        body_ws_np = wp.to_torch(body_ws_wp).numpy()

        obs_list = []
        for i in range(num_envs):
            ee_global = body_ws_np[i] + ee_idx
            ...
            target_pos_t = torch.tensor(target_pos_np[i], dtype=torch.float32, device=device_str)
            target_quat_t = torch.zeros(4, dtype=torch.float32, device=device_str)
            obs_list.append(torch.cat([jpos, jvel, eef_pos, eef_quat, target_pos_t, target_quat_t]))
        ...
        return reward_t, obs_torch, torch.zeros(num_envs, dtype=torch.bool, device=device_str)

    @staticmethod
    def backward(ctx, grad_reward, grad_obs, grad_terminated):
        # Set incoming gradient on the reward Warp array
        grad_reward_wp = wp.from_torch(grad_reward.detach().clone().contiguous(), dtype=wp.float32)
        ...
        action_grad = wp.to_torch(ctx.action_wp.grad).clone().reshape(grad_reward.shape[0], -1)
        ctx.tape.zero()
...
class FrankaReachAPGEnv:
    ...
    def __init__(
        ...
        # Body world start indices for multi-world indexing
        self.body_world_start = self.model.body_world_start.numpy()
        self.body_world_start_wp = self.model.body_world_start  # Warp array for kernels
    ...
    def step(self, action):
        ...
        sim_state = {
            ...
            "body_world_start_wp": self.body_world_start_wp,
        }
    ...
    def _get_obs_detached(self):
        ...
        obs_list = []
        for i in range(self._num_envs):
            ee_global = self.body_world_start[i] + self.ee_body_index
            ee_transform = body_q_torch[ee_global]
            eef_pos = ee_transform[:3]
            eef_quat = ee_transform[3:7]
